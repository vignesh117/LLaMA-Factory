...
@dataclass
class BlockLLMArgument:
    use_blockllm: bool = field(
        default=False,
        metadata={"help": "Whether or not to use the BlockLLM optimizer."},
    )
    # Add any other specific arguments for BlockLLM here
    blockllm_sparsity: float = field(
        default=0.5,
        metadata={"help": "The sparsity level for BlockLLM optimizer."},
    )
    blockllm_update_freq: int = field(
        default=100,
        metadata={"help": "The update frequency for BlockLLM optimizer."},
    )
...
class FinetuningArguments(
    ...
    def __post_init__(self):
        ...
        if self.finetuning_type == "lora" and (
            self.use_galore or self.use_badam or self.use_blockllm
        ):
            raise ValueError(
                "Cannot use LoRA with GaLore, BAdam, or BlockLLM together."
            )

        if self.use_galore and self.use_badam:
            raise ValueError("Cannot use GaLore with BAdam together.")

        if self.use_galore and self.use_blockllm:
            raise ValueError("Cannot use GaLore with BlockLLM together.")

        if self.use_badam and self.use_blockllm:
            raise ValueError("Cannot use BAdam with BlockLLM together.")
